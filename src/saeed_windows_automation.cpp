...
HWND FindWindowByTitle(const std::string& title){
    std::wstring n=Lower(Wide(title));HWND found=nullptr;
    std::pair<std::wstring,HWND*> ctx{n,&found};
...
std::string Title(HWND h){wchar_t b[512]{};GetWindowTextW(h,b,512);return Utf8(b);}
}
...
json EnumerateWindows(){
    ...
    EnumWindows([](HWND h,LPARAM p)->BOOL{
        ...
        a->push_back({{"title",Utf8(t)},{"pid",pid},{"x",r.left},{"y",r.top},{"width",r.right-r.left},{"height",r.bottom-r.top},{"minimized",!!IsIconic(h)},{"maximized",!!IsZoomed(h)}});
        return TRUE;
...
json SetWindowGeometry(const std::string& title,int x,int y,int width,int height,bool maximize,bool restore){
    ...
    bool verified=maximize?!!IsZoomed(h):(width>0&&height>0&&r.left==x&&r.top==y&&(r.right-r.left)==width&&(r.bottom-r.top)==height);
    ...
    try{
        for(const auto& e:std::filesystem::recursive_directory_iterator(base,std::filesystem::directory_options::skip_permission_denied,ec)){
            ...
            if(q.empty()||n.find(q)!=std::wstring::npos){
                out.push_back({{"path",Utf8(e.path().wstring())},{"directory",e.is_directory(ec)}});
                if((int)out.size()>=maxResults)break;
            }
        }
    }catch(...){}
    return {{"ok",true},{"root",Utf8(root)},{"query",Utf8(query)},{"results",out},{"count",out.size()},{"visited",visited}};
}
json EnumerateProcesses(){
    ...
    if(Process32FirstW(s,&pe)){do{out.push_back({{"name",Utf8(pe.szExeFile)},{"pid",pe.th32ProcessID}});}while(Process32NextW(s,&pe));}
    CloseHandle(s);return {{"ok",true},{"processes",out},{"count",out.size()}};
}
