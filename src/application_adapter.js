...
class ApplicationAdapter{
  ...
  async actUI(pid,action="invoke",selector={}){
    ...
    const script='Add-Type -AssemblyName UIAutomationClient;Add-Type -AssemblyName UIAutomationTypes;$proc=Get-Process -Id '+p+' -ErrorAction Stop;$root=[System.Windows.Automation.AutomationElement]::FromHandle($proc.MainWindowHandle);if(-not $root){throw "No automation root"};$cond=[System.Windows.Automation.Condition]::TrueCondition;$items=$root.FindAll([System.Windows.Automation.TreeScope]::Descendants,$cond);$target=$null;for($i=0;$i -lt $items.Count;$i++){ $el=$items.Item($i);try{$n=$el.Current.Name;$a=$el.Current.AutomationId;$t=$el.Current.ControlType.ProgrammaticName;if((("'+esc(aid)+'" -and $a -eq "'+esc(aid)+'") -or ("'+esc(name)+'" -and $n -eq "'+esc(name)+'")) -and ("'+esc(controlType)+'" -eq "" -or $t -eq "'+esc(controlType)+'")){$target=$el;break}}catch{}};if(-not $target){throw "UI element not found"};if(-not $target.Current.IsEnabled){throw "UI element is disabled"};$result=[ordered]@{action="'+esc(safeAction)+'";name=$target.Current.Name;automationId=$target.Current.AutomationId;controlType=$target.Current.ControlType.ProgrammaticName};switch("'+esc(safeAction)+'"){ "invoke" {$pat=$target.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern);$pat.Invoke();$result.performed=$true} "set_value" {$pat=$target.GetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern);$pat.SetValue("'+esc(selector.value||"")+'");$result.performed=$true;$result.value="'+esc(selector.value||"')+'"} "select" {$pat=$target.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern);$pat.Select();$result.performed=$true} "toggle" {$pat=$target.GetCurrentPattern([System.Windows.Automation.TogglePattern]::Pattern);$pat.Toggle();$result.performed=$true}};$result|ConvertTo-Json -Compress';
