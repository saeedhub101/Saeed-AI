...
class ApplicationAdapter{
  ...
  async inspectUI(pid){
    ...
    const script='Add-Type -AssemblyName UIAutomationClient;Add-Type -AssemblyName UIAutomationTypes;$proc=Get-Process -Id '+p+' -ErrorAction Stop;$root=[System.Windows.Automation.AutomationElement]::FromHandle($proc.MainWindowHandle);if(-not $root){throw "No automation root"};$walker=[System.Windows.Automation.TreeWalker]::ControlViewWalker;$items=New-Object System.Collections.Generic.List[object];$walk={param($el,$depth)if($null -eq $el -or $depth -gt 7 -or $items.Count -ge 1000){return};$name="";$type="";$aid="";$val="";$enabled=$true;$offscreen=$false;$patterns=New-Object System.Collections.Generic.List[string];$toggleState="";$selected=$false;try{$name=$el.Current.Name;$type=$el.Current.ControlType.ProgrammaticName;$aid=$el.Current.AutomationId;$enabled=$el.Current.IsEnabled;$offscreen=$el.Current.IsOffscreen}catch{};try{$pat=$el.GetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern);$val=$pat.Current.Value;$patterns.Add("ValuePattern")}catch{};try{$null=$el.GetCurrentPattern([System.Windows.Automation.InvokePattern]::Pattern);$patterns.Add("InvokePattern")}catch{};try{$null=$el.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern);$patterns.Add("SelectionItemPattern")}catch{};try{$pat=$el.GetCurrentPattern([System.Windows.Automation.TogglePattern]::Pattern);$toggleState=[string]$pat.Current.ToggleState;$patterns.Add("TogglePattern")}catch{};try{$pat=$el.GetCurrentPattern([System.Windows.Automation.SelectionItemPattern]::Pattern);$selected=[bool]$pat.Current.IsSelected}catch{};try{$pat=$el.GetCurrentPattern([System.Windows.Automation.RangeValuePattern]::Pattern);$val=if([string]::IsNullOrWhiteSpace($val)){[string]$pat.Current.Value}else{$val};$patterns.Add("RangeValuePattern")}catch{};try{$null=$el.GetCurrentPattern([System.Windows.Automation.TextPattern]::Pattern);$patterns.Add("TextPattern")}catch{};$items.Add([pscustomobject]@{name=$name;controlType=$type;automationId=$aid;value=$val;enabled=$enabled;offscreen=$offscreen;toggleState=$toggleState;selected=$selected;patterns=@($patterns);depth=$depth});$child=$walker.GetFirstChild($el);while($child){&$walk $child ($depth+1);$child=$walker.GetNextSibling($child)}};&$walk $root 0;$items|ConvertTo-Json -Depth 8 -Compress';
