...
class ApplicationAdapter{
  ...
  async inspectUI(pid){
    ...
    const script='Add-Type -AssemblyName UIAutomationClient;Add-Type -AssemblyName UIAutomationTypes;$proc=Get-Process -Id '+p+' -ErrorAction Stop;$root=[System.Windows.Automation.AutomationElement]::FromHandle($proc.MainWindowHandle);if(-not $root){throw "No automation root"};$walker=[System.Windows.Automation.TreeWalker]::ControlViewWalker;$items=New-Object System.Collections.Generic.List[object];$walk={param($el,$depth)if($null -eq $el -or $depth -gt 6 -or $items.Count -ge 500){return};$name="";$type="";$aid="";$val="";try{$name=$el.Current.Name;$type=$el.Current.ControlType.ProgrammaticName;$aid=$el.Current.AutomationId}catch{};try{$pat=$el.GetCurrentPattern([System.Windows.Automation.ValuePattern]::Pattern);$val=$pat.Current.Value}catch{};$items.Add([pscustomobject]@{name=$name;controlType=$type;automationId=$aid;value=$val;depth=$depth});$child=$walker.GetFirstChild($el);while($child){&$walk $child ($depth+1);$child=$walker.GetNextSibling($child)}};&$walk $root 0;$items|ConvertTo-Json -Depth 6 -Compress';
