...
const head=part(new THREE.SphereGeometry(.46,32,20),[0,1.82,0],[1,1.08,.95]);
...
loadAvatar();
window.saeedAvatar={setMood(mood){root.rotation.z=0;root.position.y=mood==="sleep"?-.05:0},play(name){if(!mixer)return;const c=clips.find(x=>x.name.toLowerCase().includes(String(name).toLowerCase()));if(c)mixer.clipAction(c).reset().play()}};
