...
class OfficeTools{
 ...
 async excel(action,args={}){
  ...
  if(action==="inspect"){
   ...
   body=`$p=${file};$wsName=${sheet};$rows=ConvertFrom-Json '${rows.replace(/'/g,"''")}';$xl=$null;$wb=$null;try{$xl=New-Object -ComObject Excel.Application;$xl.Visible=$false;$wb=$xl.Workbooks.Open((Resolve-Path $p).Path,$false,$false);$ws=$wb.Worksheets.Item($wsName);$next=$ws.UsedRange.Row+$ws.UsedRange.Rows.Count;if($ws.UsedRange.Count -eq 1 -and [string]::IsNullOrWhiteSpace([string]$ws.UsedRange.Value2)){$next=1};$r=$next;foreach($row in $rows){$c=1;foreach($v in $row){$ws.Cells.Item($r,$c).Value2=$v;$c++};$r++};$wb.Save();$check=@();for($i=0;$i -lt $rows.Count;$i++){ $row=$rows[$i];$vals=@();for($j=0;$j -lt $row.Count;$j++){ $vals+=$ws.Cells.Item($next+$i,$j+1).Value2 };$check+=[pscustomobject]@{row=$next+$i;values=$vals} };[pscustomobject]@{ok=$true,rowsAdded=$rows.Count,startRow=$next,verifiedRows=$check}|ConvertTo-Json -Depth 6 -Compress}finally{if($wb){$wb.Close($false)};if($xl){$xl.Quit()}}`;
 ...
 async word(action,args={}){
  ...
  const script=action==="read_text"
   ...
   : `$p=${file};$w=New-Object -ComObject Word.Application;$d=$w.Documents.Open((Resolve-Path $p).Path);$f=$d.Content.Find;$f.ClearFormatting();$f.Replacement.ClearFormatting();$f.Text=ConvertFrom-Json '${find}';$f.Replacement.Text=ConvertFrom-Json '${repl}';$null=$f.Execute(\$true,\$false,\$false,\$false,\$false,\$false,\$true,1,\$false,\$null,2);$d.Save();$d.Close();$w.Quit();$after=$d.Content.Text;[pscustomobject]@{ok=$true,path=$p,replacementVerified=($after -notlike ('*'+(ConvertFrom-Json '${find}')+'*'))}|ConvertTo-Json -Compress`;
